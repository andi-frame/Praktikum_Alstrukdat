...
boolean IsCIRCLEValid(float X, float Y, float R) {
  ...
  return !isnan(X) && !isnan(Y) && !isnan(R) && !isinf(X) && !isinf(Y) &&
         !isinf(R);
}
...
float Circumference(CIRCLE C) { return 2 * 3.14 * C.R; }

float Area(CIRCLE C) { return 3.14 * C.R * C.R; }

boolean IsPOINTInsideCIRCLE(CIRCLE C, POINT P) {
  float distance = sqrt((C.CENTER.X - P.X) * (C.CENTER.X - P.X) +
...
boolean IsPOINTInEdgeCIRCLE(CIRCLE C, POINT P) {
  float distance = sqrt((C.CENTER.X - P.X) * (C.CENTER.X - P.X) +
                        (C.CENTER.Y - P.Y) * (C.CENTER.Y - P.Y));
  if ((distance <= C.R + 0.000001) && (distance >= C.R - 0.000001)) {
    return true;
  } else {
    return false;
  }
...
boolean IsCIRCLESTangents(CIRCLE C1, CIRCLE C2) {
  float distance =
      sqrt((C1.CENTER.X - C2.CENTER.X) * (C1.CENTER.X - C2.CENTER.X) +
           (C1.CENTER.Y - C2.CENTER.Y) * (C1.CENTER.Y - C2.CENTER.Y));
  float sumR = C1.R + C2.R;
  if ((distance <= sumR + 0.000001) && (distance >= sumR - 0.000001)) {
    return true;
  } else if ((distance + C1.R <= C2.R + 0.000001) &&
                 (distance + C1.R >= C2.R - 0.000001) ||
             (distance + C2.R <= C1.R + 0.000001) &&
                 (distance + C2.R >= C1.R - 0.000001)) {
    return true;
  } else {
    return false;
